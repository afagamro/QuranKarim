...
class MadaniPageProvider(private val context: Context) {
    // هذه المصادر تنشر ملف MP3 واحدًا لكل سورة بصيغة SSS.mp3.
    private val wholeSurahQariIds: Set<String> = setOf(
        "mohammad_alluhaidan_full",
        "idris_abkar_full",
        "khalid_aljalil_full",
        "raad_alkurdi_full",
        "ahmed_alnufais_full",
        "salah_alhashim_full",
        "waleed_alnaihi_full",
        "ahmed_altrabulsi_full",
        "abdulrahman_alousi_full",
        "abdulrahman_alshahhat_full",
        "abdulaziz_alzahrani_full",
        "abdulwadud_hanif_full",
        "mahmoud_alrifai_full",
        "alzain_mohamed_ahmed_full",
        "nourin_mohamed_siddig_full",
        "ahmed_mohamed_taher_full"
    )


    /** نحمل القوائم من resources (الأسماء تُترجم حسب اللغة) */
    private val allQariList: List<Qari> by lazy {
    ...
    /** قائمة العرض: اسم واحد لكل قارئ، بلا أرقام جودة أو نسخ مكررة. */
    private val qariList: List<Qari> by lazy {
        val seen = HashSet<String>()
        allQariList
            .map { it.copy(name = cleanQariName(it.name)) }
            .filter { seen.add(qariGroupKey(it.id, it.name)) }
    }

    private fun cleanQariName(name: String): String =
        name.replace(Regex("""\s*\([^)]*\)\s*$"""), "").trim()

    private fun qariGroupKey(id: String, cleanName: String): String = when {
        id.startsWith("abdulbasit_") || id == "abdulsamad_qe64" -> "abdulbasit"
        id.startsWith("minshawy_") -> "minshawy"
        id.startsWith("husary_") -> "husary"
        id.startsWith("basfar") -> "basfar"
        id == "rifai_hani" || id == "hani_rifai192" -> "hani_rifai"
        id == "hudhaify" || id == "hudhaify128" -> "hudhaify"
        else -> cleanName.lowercase()
    }
    ...
    fun getQariById(id: String): Qari? {
        val qari = allQariList.find { it.id == canonicalQariId(id) } ?: return null
        return qari.copy(name = cleanQariName(qari.name))
    }

    /** ترحيل معرفات نسخ الجودة القديمة إلى القارئ الوحيد الظاهر حاليًا. */
    fun canonicalQariId(id: String): String = when (id.trim().lowercase()) {
        "abdulbasit_murattal192", "abdulbasit_mujawwad128", "abdulsamad_qe64" ->
            "abdulbasit_murattal64"
        "minshawy_mujawwad64", "minshawy_teacher128" -> "minshawy_murattal128"
        "husary_128", "husary_muallim128", "husary_mujawwad64" -> "husary_64"
        "basfar192" -> "basfar64"
        "hani_rifai192" -> "rifai_hani"
        "hudhaify128" -> "hudhaify"
    ...
    /** بعض المصادر الموثوقة تنشر ملفًا واحدًا لكل سورة بدل ملف لكل آية. */
    fun isWholeSurahQari(id: String): Boolean =
        id.trim().lowercase() in wholeSurahQariIds

    fun isWholeSurahQari(qari: Qari): Boolean = isWholeSurahQari(qari.id)
    ...
    /** رابط ملف السورة الكامل للمصادر التي لا توفر ملفات منفصلة للآيات. */
    fun getSurahUrl(qariId: String, surah: Int): String? {
        val q = getQariById(qariId) ?: return null
        return getSurahUrl(q, surah)
    }

    fun getSurahUrl(qari: Qari, surah: Int): String {
        val s = surah.coerceIn(1, 114).toString().padStart(3, '0')
        return "${qari.url}$s.mp3"
    }
    ...
    fun getUrlsForPage(qariId: String, page: Int): List<String> {
        ...
        if (isWholeSurahQari(qari)) {
            return ranges.map { it.surah }.distinct().map { getSurahUrl(qari, it) }
        }
    ...
    fun getUrlsForSurah(qariId: String, surah: Int): List<String> {
        val qari = getQariById(qariId) ?: return emptyList()
        if (isWholeSurahQari(qari)) return listOf(getSurahUrl(qari, surah))
